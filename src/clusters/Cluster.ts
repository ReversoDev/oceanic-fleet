import { ClusterConnectMessage } from "./../util/Queue.js";
import * as Oceanic from "oceanic.js";
import cluster from "node:cluster";
import {BaseClusterWorker} from "./BaseClusterWorker.js";
import {inspect} from "node:util";
import {LoggingOptions, StartingStatus, ShardStats} from "../sharding/Admiral.js";
...
export class Cluster {
			...
			if (message.op) {
				...
				case "collectStats": {
					...
					if (process.send) process.send({op: "collectStats", stats: {
						...
						requestHandlerLatencyRef: this.useCentralRequestHandler ? this.bot.rest.handler.latencyRef : undefined
					}});
				...
				case "shutdown": {
					...
					if (this.app) {
						if (this.app.shutdown) {
							// Ask app to shutdown
							this.app.shutdown(() => {
								this.bot.shards.forEach(s => s.disconnect(false));
	...
	private async connect() {
		...
		if (this.BotWorker) {
			App = this.BotWorker;
			bot = new this.oceanicClient({ auth: this.token, ...options });
		} else {
			try {
				App = await import(this.path!);
				if (App.Oceanic) {
					bot = new App.Oceanic.Client({ auth: this.token, ...options });
					App = App.BotWorker;
				} else {
					bot = new this.oceanicClient({ auth: this.token, ...options });
					if (App.BotWorker) {
