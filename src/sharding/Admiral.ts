import primary, { Worker } from "node:cluster";
...
import { Queue, QueueItem, ClusterConnectMessage, ServiceConnectMessage } from "../util/Queue.js";
import { errorToJSON, parseJSON, reconstructError, stringifyJSON } from "../util/Serialization.js";
import { BaseClusterWorker } from "../clusters/BaseClusterWorker.js";
import { BaseServiceWorker } from "../services/BaseServiceWorker.js";
import { IPC, IpcHandledLog } from "../util/IPC.js";
...
export class Admiral extends EventEmitter {
	...
	*/
	public constructor(options: Options) {
		...
		this.clientOptions = options.clientOptions ?? {gateway: { intents: Oceanic.AllNonPrivilegedIntents }};
		...
		this.oceanic = new this.oceanicClient({ auth: this.token });

		this.launch();

		if (primary.isPrimary) {
			primary.on("message", (worker, message) => {
				if (message.op) {
					switch (message.op) {
						case "launched": {
			...
			primary.on("exit", (worker/*, code, signal*/) => {
				const cluster = this.clusters.find((c: ClusterCollection) => c.workerID === worker.id);
				const service = this.services.find((s: ServiceCollection) => s.workerID === worker.id);
				const name = () => {
					if (cluster) {
			...
			this.queue.on("execute", (item: QueueItem/*, prevItem?: QueueItem*/) => {
				const worker = primary.workers![item.workerID];
				if (worker) {
	...
	private ipcMessageHandler(worker: FakeWorker, message: any) {
		...
		case "serviceCommand": {
			...
			if (service) {
				const serviceWorker = primary.workers![service.workerID];
				if (serviceWorker) {
					serviceWorker.send({
						op: "command",
		...
		case "clusterCommand": {
			...
			if (cluster) {
				const clusterWorker = primary.workers![cluster.workerID];
				if (clusterWorker) {
					clusterWorker.send({
						op: "command",
						command: message.command,
		...
		case "allClustersCommand": {
			this.clusters.forEach((c: ClusterCollection) => {
				const clusterWorker = primary.workers![c.workerID];
				if (clusterWorker) {
		...
		case "sendTo": {
			...
			const worker = primary.workers![clusterObj.workerID];
			if (!worker) return;
	...
	private ipcReturn(message: {UUID: string | number, value: {id: string | symbol, value: any, noValue?: boolean}}) {
		const sendReturn = (value: any) => {
			if (message.UUID === "master") {
				...
				const requestingWorker = primary.workers![message.UUID];
				if (requestingWorker) {
	...
	private centralApiRequest(worker: Worker, UUID: string, data: {method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE", url: string, auth?: boolean, body?: { [s: string]: unknown }, file?: Oceanic.File, fileString?: string, _route?: string, short?: boolean}) {
		...
		if (data.fileString && data.file) {
			data.file.contents = Buffer.from(data.fileString, "base64");
		}


		//@ts-ignore
		this.oceanic.rest.request({ method: data.method, auth: data.auth, files: [ data.file ], path: data.url, route: data._route, json: data.body  })
			.then((value) => {
				reply(true, value);
			})
			.catch((error) => {
	...
	*/
	public restartCluster(clusterID: number, hard: boolean): void {
		...
		const worker = primary.workers![workerID];
		if (worker) {
	...
	public totalShutdown(hard: boolean): void {
		...
		if (hard) {
			...
			this.clusters.forEach((cluster) => {
				total++;
				process.nextTick(() => {
					const worker = primary.workers![cluster.workerID];
					if (worker) {
			...
			this.services.forEach((service) => {
				total++;
				process.nextTick(() => {
					const worker = primary.workers![service.workerID];
					if (worker) {
						const shutdownItem = this.shutdownWorker(worker, hard ? false : true, doneFn);
						queueItems.push(shutdownItem);
						checkCompleted();
		...
		if (!this.resharding) {
			...
			this.once("ready", () => {
				...
				oldClusters.forEach((c) => {
					const oldWorker = primary.workers![c.workerID];
					if (oldWorker) {
						const shutdownItem = this.shutdownWorker(oldWorker, true, () => {
							...
							if (newWorkerClusterObj) {
								newWorker = primary.workers![newWorkerClusterObj.workerID];
							}
							...
							if (i === oldClusters.size) {
								...
								this.clusters.forEach((c: ClusterCollection) => {
									if (!oldClusters.get(c.clusterID)) {
										const newWorker = primary.workers![c.workerID];
										if (newWorker) newWorker.send({op: "loadCode"});
										if (this.whatToLog.includes("resharding_transition")) {
